...
static void print2byte(uint16_t input, struct iio_channel_info *info)
{
    ...
    lv_lock();
	lv_subject_set_int(&api_hmi->subjects.temperature_indoor, input);
    lv_subject_set_int(&api_hmi->subjects.temperature_outdoor, input-65);
    lv_unlock();
    publish_sensor_data(input/10.0);
}
...
static enum autochan autochannels = AUTOCHANNELS_ENABLED;
